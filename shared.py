...
def top_bar(links=None, active=None):
    if links is None:
        links = [
    ...
    st.markdown(f'<div class="top-bar">{links_html}</div>', unsafe_allow_html=True)


def setup_page(show_top_bar=True, top_bar_links=None, active_page=None):
    st.set_page_config(layout="wide")
    with open("style.css") as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
    if show_top_bar:
        top_bar(top_bar_links, active_page)
