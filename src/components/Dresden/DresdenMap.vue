...
import BerlinMap from '@img/Dresden/BerlinMap.svg?raw'
import CologneMap from '@img/Dresden/CologneMap.svg?raw'
import DortmundMap from '@img/Dresden/DortmundMap.svg?raw'
import DresdenMap from '@img/Dresden/DresdenMap.svg?raw'
import DusseldorfMap from '@img/Dresden/DusseldorfMap.svg?raw'
import FrankfurtMap from '@img/Dresden/FrankfurtMap.svg?raw'
import HamburgMap from '@img/Dresden/HamburgMap.svg?raw'
import LeipzigMap from '@img/Dresden/LeipzigMap.svg?raw'
import MunichMap from '@img/Dresden/MunichMap.svg?raw'
import StuttgartMap from '@img/Dresden/StuttgartMap.svg?raw'
...
const cityMaps = {
    Berlin: BerlinMap,
    Cologne: CologneMap,
    Dortmund: DortmundMap,
    Dresden: DresdenMap,
    Dusseldorf: DusseldorfMap,
    Frankfurt: FrankfurtMap,
    Hamburg: HamburgMap,
    Leipzig: LeipzigMap,
    Munich: MunichMap,
    Stuttgart: StuttgartMap
}

const mapHost = ref(null)
const mapSvgRaw = computed(() => {
    return cityMaps[props.selectedCity] || DresdenMap
})
...
function selectRegion(group) {
    ...
    // Then emit to Streamlit (slow round-trip)
...
function findRegionGroup(target) {
    ...
    // Skip the root map wrapper (e.g. HamburgMap, DresdenMap)
    if (group.id.endsWith('Map')) return null
...
watch(
    ...
    async (newCity, oldCity) => {
        ...
        const regions = mapHost.value?.querySelectorAll('g[id]')
...
<template>
    <div class="dresden-map-host" :class="{ 'is-edit': props.activeMode === 'edit' }">
        ...
        <div ref="mapHost" class="dresden-map-inner" v-html="mapSvgRaw" />
    </div>
</template>
...
.dresden-map-inner :deep(.dresden-map) {
    display: block;
    width: auto;
    height: 100%;
    overflow: visible;
...
.dresden-map-inner :deep(svg) {
    height: 100%;
    width: fit-content;
}

.dresden-map-inner :deep(.map-region) {
    cursor: pointer;
    outline: none;
}

.dresden-map-inner :deep(.map-region > path:first-of-type) {
...
.dresden-map-inner :deep(.map-region:hover > path:first-of-type),
.dresden-map-inner :deep(.map-region:focus-visible > path:first-of-type) {
    fill: rgba(255, 183, 3, 0.22);
    stroke: var(--orange-dark);
    stroke-width: 4;
}

.dresden-map-inner :deep(.map-region.is-selected > path:first-of-type) {
